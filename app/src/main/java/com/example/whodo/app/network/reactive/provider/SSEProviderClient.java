package com.example.whodo.app.network.reactive.provider;

import android.util.Log;
import com.example.whodo.app.ViewModelInterface;
import com.example.whodo.app.network.reactive.SSEClientInstanceProvider;
import com.example.whodo.app.network.reactive.loggedUser.SSELoggedUserCallback;

import okhttp3.Call;
import okhttp3.OkHttpClient;
import okhttp3.Request;

public class SSEProviderClient {
    private final OkHttpClient client;
    private static final String TAG = "LOGGER-SSE-PROV-Client";

    private Call currentCall;
    public SSEProviderClient() {
        this.client = SSEClientInstanceProvider.getInstance();
    }
    public void startListening(ViewModelInterface pViewModel, String SSE_URL) {
        Log.d(TAG, "SSEProviderClient Iniciando conexión SSE con: " + SSE_URL);
        Request request = new Request.Builder().url(SSE_URL).build();
        currentCall = client.newCall(request);
        currentCall.enqueue(new SSEProviderCallback(pViewModel));
    }
    public void stopListening() {
        if (currentCall != null && !currentCall.isCanceled()) {
            currentCall.cancel();
            Log.d(TAG, "SSE connection cancelled");
        }
    }
}

